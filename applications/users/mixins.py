...
class SigPermisoMixin(LoginRequiredMixin):
    login_url = reverse_lazy('users_app:user-login')
    ...
    def dispatch(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            return self.handle_no_permission()
        #
        if not check_ocupation_user(request.user.ocupation, User.SIG):
            # no tiene autorizacion
            return HttpResponseRedirect(
                reverse(
                    'users_app:user-login'
...
class MensajeroPermisoMixin(LoginRequiredMixin):
    login_url = reverse_lazy('users_app:user-login')
    ...
    def dispatch(self, request, *args, **kwargs):
        if not request.user.is_authenticated:
            return self.handle_no_permission()
        #
        if not check_ocupation_user(request.user.ocupation, User.COURRIER):
            # no tiene autorizacion
            return HttpResponseRedirect(
                reverse(
                    'users_app:user-login'
